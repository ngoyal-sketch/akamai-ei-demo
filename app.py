...
AKAMAI_CSS = """
<style>
    /* 1. Tighter Global Spacing */
    .block-container { padding: 1.5rem 2rem !important; }
    header { display: none !important; }
    
    /* 2. Override Streamlit's default widget gaps for a denser dashboard feel */
    div[data-testid="stVerticalBlock"] > div { padding-bottom: 0.2rem !important; }
    .stTextArea > label { margin-bottom: -5px !important; }
    .stSelectbox > label { margin-bottom: -5px !important; }
    ...
    /* 3. Tighter Topbar */
    .akamai-topbar {
        background-color: #1E2228; color: #FFFFFF; padding: 8px 24px; 
        margin-top: -40px; margin-left: -60px; margin-right: -60px; margin-bottom: 15px;
    ...
    .akamai-search-box { background-color: #2B313A; border: 1px solid #3A424D; border-radius: 4px; padding: 4px 12px; color: #C0C7D0; width: 350px; font-size: 12px; }
    .akamai-top-right { display: flex; align-items: center; gap: 16px; font-size: 12px; color: #E2E8F0; }
    .create-btn { background-color: #0072CE; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 12px; }
    ...
    .akamai-card-title { font-size: 16px; font-weight: 700; color: #1E2228; margin-bottom: 10px; }
    
    .empty-state-box { background-color: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 4px; padding: 40px 20px; text-align: center; margin-top: 5px; }
    .empty-state-title { font-size: 15px; font-weight: 700; color: #2B313A; margin-bottom: 6px; }
    .empty-state-sub { font-size: 12px; color: #64748B; }
    
    .stButton > button { background-color: #0072CE !important; color: #FFFFFF !important; font-weight: 600 !important; border-radius: 4px !important; border: none !important; padding: 6px 14px !important; font-size: 13px !important; }
    
    /* 5. Tighter Text & Headers */
    .section-header { font-size: 14px; font-weight: 700; color: #1E2228; margin-top: 12px; margin-bottom: 5px; border-bottom: 1px solid #E2E8F0; padding-bottom: 5px;}
    ul { margin-top: 6px; padding-left: 20px; color: #2B313A; font-size: 13px; margin-bottom: 8px; }
...
SVG_CART = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>'
...
def run_track_2_analysis(industry, region, business_issue):
    issue_lower = business_issue.lower()
    observations = [
        f"Data Privacy Track 2 Active: Internal configurations bypassed. Analysis is based on {industry} sector telemetry in {region}.",
...
st.markdown("<p style='font-size: 13px; margin-top: -15px; margin-bottom: 15px; color: #64748B;'>Akamai EI - Akamai EdgeIntelligence Marketplace</p>", unsafe_allow_html=True)

# 🔥 COMPACTED LIMITED AVAILABILITY BANNER
st.markdown("""
<div style="background-color: #E6F4EA; border-left: 4px solid #137333; padding: 12px; margin-bottom: 15px; border-radius: 4px;">
    <h4 style="margin-top: 0; margin-bottom: 6px; color: #137333; font-size: 15px;">✨ Early Access: Explore Akamai's Latest Innovations</h4>
    <p style="font-size: 12px; color: #2B313A; margin-bottom: 6px;">Discover our newest LA (Limited Availability) products designed to protect and optimize your environment in the AI era.</p>
    <ul style="font-size: 12px; color: #2B313A; padding-left: 15px; margin-bottom: 0;">
        <li style="margin-bottom: 4px;"><b>AI Brand Presence:</b> Optimizes how AI systems access and interpret content, preserving brand visibility across AI-driven experiences. <a href="#">Read or initiate try here!</a></li>
        <li style="margin-bottom: 4px;"><b>Guardicore Segmentation:</b> The first AI-powered segmentation platform with built-in Exposure Analysis & Response capabilities. <a href="#">Read or initiate try here!</a></li>
        <li style="margin-bottom: 4px;"><b>API Security:</b> Protect APIs throughout their entire lifecycle to reduce the risk of data breaches. <a href="#">Read or initiate try here!</a></li>
...
if gateway_choice == "I know which Akamai product I want to buy or trial.":
    ...
    with col1:
        ...
        st.markdown("<span style='font-size: 13px; font-weight: 600; color: #1E2228;'>Data Privacy & Analysis Track:</span>", unsafe_allow_html=True)
        ...
        captions=[
            ...
            "No config scan. AI suggests products based on global industry threat benchmarks."
        ],
        ...
        if "Track 1" in track_choice:
            st.markdown("<p style='font-size: 12px; color: #0072CE; margin-bottom: 8px;'>Provide Delivery and/or Security configurations for analysis.</p>", unsafe_allow_html=True)
            input_method = st.radio("Configuration Source:", ["Select from Catalog", "Paste Custom JSON"], horizontal=True, label_visibility="collapsed")
            ...
            if input_method == "Select from Catalog":
                ...
                final_del_payload = st.text_area("Paste Delivery JSON (Property Manager):", height=100)
                final_sec_payload = st.text_area("Paste Security JSON (AppSec):", height=100)
        else:
            st.markdown("<p style='font-size: 12px; color: #0072CE; margin-bottom: 8px;'>Deep-Insight Scanning Disabled. Using Global Intelligence benchmarks.</p>", unsafe_allow_html=True)
        ...
        st.markdown('<div class="akamai-card-title" style="margin-top:15px;">2. Business Context (Copilot)</div>', unsafe_allow_html=True)
        issue_input = st.text_area("Describe what you are looking to enhance or any issues you are currently facing:", placeholder="e.g., Credential stuffing attacks are locking out real users, or looking to reduce origin latency.", height=68)
    ...
    with col2:
        if run_scan and issue_input:
            ...
            <div class="empty-state-box">
                ...
                <p class="empty-state-sub">👈 After you provide all the details, the recommendations will be shown here.</p>
            </div>
            """, unsafe_allow_html=True)
